  ...
  try {
    ...
    if (body.action === 'deposit' || body.action === 'withdraw') {
      ...
      const txn = await postTransaction({
        ...
        type: body.action === 'deposit' ? 'BANK_DEPOSIT' : 'BANK_WITHDRAWAL',
        category: body.action === 'deposit' ? 'BANK_DEPOSIT' : 'BANK_WITHDRAWAL',
    ...
    return NextResponse.json({ account: bank }, { status: 201 });
  } catch (err) {
